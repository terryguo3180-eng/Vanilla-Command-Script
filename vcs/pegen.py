...
A single-file module of https://github.com/we-like-parsers/pegen.

The original version only supports the official python tokenizer,
so for project compatibility this module was heavily modified and
contains a lot of extra features.
"""
...
class PythonParserGenerator(ParserGenerator, GrammarVisitor):
    ...
    def generate(self, filename: str, output: str) -> None:
        with open("vcs/parser_template.py") as f:
            template = f.read()
        start_marker = "# __START_FILE__"
        template = template[template.find(start_marker) + len(start_marker):].lstrip()
        ...
        code_marker = "# __PARSER_CODE__"
        code_idx = template.find(code_marker)
        header = template[:code_idx]
        trailer = template[code_idx + len(code_marker):]

        def interp(code: str):
            return (
                code.replace("__FILENAME__", filename)
                    .replace("__OUTPUT__", output)
                    .replace("__CLASS_NAME__", cls_name)
                    .replace("__FALLBACK__", fallback)
                    .replace("# __SUBHEADER__", subheader)
            ).rstrip()

        self.print(interp(header))
        while self.todo:
